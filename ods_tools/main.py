...
def add_exposure_data_args(command):
    ...
    command.add_argument(
        "--oed-dir",
        help="Path to the OED directory containing stardard named OED files",
        default=None,
    )
    command.add_argument(
        "--config-json", help="Path to the config_json file", default=None
    )
    command.add_argument(
        "--validation-config", help="Path to the validation_config file", default=None
    )
...
command_parser = main_parser.add_subparsers(
    help="command [convert]", dest="command", required=True
)
convert_command = command_parser.add_parser(
    "convert",
    description=convert_description + oed_exposure_creation,
    formatter_class=argparse.RawTextHelpFormatter,
)
add_exposure_data_args(convert_command)
convert_command.add_argument(
    "--check-oed",
    help="if True, OED file will be checked before convertion",
    default=False,
)
convert_command.add_argument(
    "--output-dir", help="path of the output directory", required=False
)
convert_command.add_argument(
    "-c",
    "--compression",
...
convert_command.add_argument(
    "--save-config",
    help="if True, OED config file will be save in the --path directory",
    default=False,
)
convert_command.add_argument(
    "-v",
...
convert_command.add_argument(
    "-vsc",
    "--version",
    help="specific OED version to use in the conversion",
    default=None,
    type=str,
)
...
check_command = command_parser.add_parser(
    "check",
    description=check_description + oed_exposure_creation,
    formatter_class=argparse.RawTextHelpFormatter,
)
add_exposure_data_args(check_command)
check_command.add_argument(
    "--model-settings-json",
    help="Path to Model settings meta-data file to check",
    default=None,
)
check_command.add_argument(
    "--analysis-settings-json",
    help="Path to Analysis settings file to check",
    default=None,
)
check_command.add_argument(
    "-v",
