...
def assign_exceedance_probability(df, max_period):
    ...
    df["rank"] = (df.groupby(by=["group_set_id", "SummaryId", "EPCalc"], as_index=False)["Loss"]
...
def generate_ep(gplt, max_group_period, oep=True, aep=True):
    ...
    grouped_df = grouped_df.groupby(by=["group_set_id", "SummaryId", "GroupPeriod", "EPCalc"], as_index=False)
