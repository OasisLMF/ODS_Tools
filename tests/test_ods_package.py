...
class OdsPackageTests(TestCase):
    ...
    def test_load_oed_from_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_run_dir:
            ...
            for compression in ["csv", "parquet", "zip"]:
                exposure.location.save(
                    f"oed_test_{compression}",
                    {
                        "source_type": "filepath",
                        "filepath": os.path.join(
                            tmp_run_dir, "oed_test", f"location.{compression}"
                        ),
                    },
                )

            # check you can reload
            exposure_json_fp = os.path.join(
                tmp_run_dir, "oed_test", OedExposure.DEFAULT_EXPOSURE_CONFIG_NAME
            )
            ...
            self.assertTrue(
                exposure.location.dataframe.equals(exposure2.location.dataframe)
            )
    ...
    def test_load_oed_from_df(self):
        ...
        self.assertTrue(
            pd.api.types.is_numeric_dtype(exposure.location.dataframe["BuildingTIV"])
        )
    ...
    def test_load_oed_from_stream(self):
        with tempfile.TemporaryDirectory() as tmp_run_dir:
            ...
            with open(
                os.path.join(tmp_run_dir, "SourceAccOEDPiWind.parquet"), "wb"
            ) as acc_parquet:
                acc_parquet.write(
                    urllib.request.urlopen(
                        base_url + "/SourceAccOEDPiWind.parquet"
                    ).read()
                )

            config = {
                "location": urllib.request.urlopen(
                    base_url + "/SourceLocOEDPiWind.csv"
                ),
                "account": {
                    "oed_info": open(
                        os.path.join(tmp_run_dir, "SourceAccOEDPiWind.parquet"), "rb"
                    ),
                    "format": "parquet",
                },
                "use_field": True,
        ...
        with tempfile.TemporaryDirectory() as tmp_run_dir:
            ...
            for extension in ["csv", "parquet"]:
                ...
                for oed_type, oed_name in OED_TYPE_TO_NAME.items():
                    configs_from_file[extension][oed_name] = os.path.join(
                        tmp_run_dir, f"Source{oed_type}OEDPiWind.{extension}"
                    )
                    with open(
                        configs_from_file[extension][oed_name], "wb"
                    ) as acc_parquet:
                        acc_parquet.write(
                            urllib.request.urlopen(
                                base_url + f"/Source{oed_type}OEDPiWind.{extension}"
                            ).read()
                        )
                    configs_from_stream[extension][oed_name] = open(
                        os.path.join(
                            tmp_run_dir, f"Source{oed_type}OEDPiWind.{extension}"
                        ),
                        "rb",
            ...
            pd.testing.assert_frame_equal(
                exposure_csv_file.location.dataframe,
                exposure_parquet_file.location.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.account.dataframe,
                exposure_parquet_file.account.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.ri_info.dataframe,
                exposure_parquet_file.ri_info.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.ri_scope.dataframe,
                exposure_parquet_file.ri_scope.dataframe,
                check_categorical=False,
            )

            pd.testing.assert_frame_equal(
                exposure_csv_file.location.dataframe,
                exposure_csv_stream.location.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.account.dataframe,
                exposure_csv_stream.account.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.ri_info.dataframe,
                exposure_csv_stream.ri_info.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.ri_scope.dataframe,
                exposure_csv_stream.ri_scope.dataframe,
                check_categorical=False,
            )

            pd.testing.assert_frame_equal(
                exposure_csv_file.location.dataframe,
                exposure_parquet_stream.location.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.account.dataframe,
                exposure_parquet_stream.account.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.ri_info.dataframe,
                exposure_parquet_stream.ri_info.dataframe,
                check_categorical=False,
            )
            pd.testing.assert_frame_equal(
                exposure_csv_file.ri_scope.dataframe,
                exposure_parquet_stream.ri_scope.dataframe,
                check_categorical=False,
            )
    ...
    def test_load_oed_from_stream__detect_type(self):
        with tempfile.TemporaryDirectory() as tmp_run_dir:
            # read_parquet needs stream with seek method which urllib.request.urlopen doesn't have
            with open(
                os.path.join(tmp_run_dir, "SourceLocOEDPiWind.csv"), "wb"
            ) as loc_csv:
                loc_csv.write(
                    urllib.request.urlopen(base_url + "/SourceLocOEDPiWind.csv").read()
                )
            with open(
                os.path.join(tmp_run_dir, "SourceAccOEDPiWind.parquet"), "wb"
            ) as acc_parquet:
    ...
    def test_load_oed_from_stream__invalid_types(self):
        with tempfile.TemporaryDirectory() as tmp_run_dir:
            # read_parquet needs stream with seek method which urllib.request.urlopen doesn't have
            with open(
                os.path.join(tmp_run_dir, "SourceLocOEDPiWind.parquet"), "wb"
            ) as loc_csv:
                loc_csv.write(
                    urllib.request.urlopen(base_url + "/SourceLocOEDPiWind.csv").read()
                )
            with open(
                os.path.join(tmp_run_dir, "SourceAccOEDPiWind.csv"), "wb"
            ) as acc_parquet:
                acc_parquet.write(
                    urllib.request.urlopen(
                        base_url + "/SourceAccOEDPiWind.parquet"
            ...
            csv_as_parquet = open(
                os.path.join(tmp_run_dir, "SourceLocOEDPiWind.parquet"), "rb"
            )
    ...
    def test_reporting_currency(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_run_dir:
            ...
            pd.testing.assert_frame_equal(
                exposure.location.dataframe, exposure2.location.dataframe
            )
            pd.testing.assert_frame_equal(
                exposure.account.dataframe, exposure2.account.dataframe
            )
            pd.testing.assert_frame_equal(
                exposure.ri_info.dataframe, exposure2.ri_info.dataframe
            )
            pd.testing.assert_frame_equal(
                exposure.ri_scope.dataframe, exposure2.ri_scope.dataframe
            )
    ...
    def test_convert_to_parquet(self):
        with tempfile.TemporaryDirectory() as tmp_run_dir:
            ...
            convert(
                config_json=str(pathlib.Path(tmp_run_dir, "config.json")),
                    ...
                    self.assertTrue(
                        os.path.isfile(
                            pathlib.Path(
                                tmp_run_dir,
                                folder,
                                f"Source{oed_name}OEDPiWind.parquet",
                            )
                        )
                    )
    ...
    def test_validation_raise_exception(self):
        ...
        exposure.location.dataframe[
            "CondTag"
        ] = "1"  # check CR only apply to relevant source type
        ...
        with pytest.raises(OdsException) as e:
            ...
            self.assertTrue(
                "column 'LayerParticipation' has values outside range." in e.msg
            )
            self.assertTrue("ReinsPeril has invalid perils." in e.msg)
            self.assertTrue(
                "Conditionally required column missing" in e.msg and "LocPeril" in e.msg
            )
    ...
    def test_load_non_utf8(self):
        ...
        exposure = OedExposure(**config)
        self.assertTrue(
            exposure.location.dataframe["StreetAddress"][0]
            == "Ô, Avenue des Champs-Élysées"
        )
    ...
    def test_aliases_columns(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_run_dir:
            ...
            with open(OedSchema.DEFAULT_ODS_SCHEMA_PATH) as default_schema_file, open(
                custom_schema_path, "w"
            ) as custom_schema_file:
                default_schema = json.load(default_schema_file)
                default_schema["input_fields"]["Loc"]["locnumber"][
                    "alias"
                ] = "LocNumberAlias"
            ...
            pd.testing.assert_series_equal(
                exposure.location.dataframe["LocNumber"],
    ...
    def test_load_exposure_from_different_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_save_dir, tempfile.TemporaryDirectory() as tmp_move_dir:
            ...
            shutil.move(
                str(pathlib.Path(tmp_save_dir, "oed")), str(pathlib.Path(tmp_move_dir))
            )

            exposure_move = OedExposure.from_dir(pathlib.Path(tmp_move_dir, "oed"))
            pd.testing.assert_frame_equal(
                exposure_save.location.dataframe, exposure_move.location.dataframe
            )
            pd.testing.assert_frame_equal(
                exposure_save.account.dataframe, exposure_move.account.dataframe
            )
    ...
    def test_field_required_allow_blank_are_set_to_default(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert (
                modified_exposure.location.dataframe["ContentsTIV"] == 0
            ).all()  # check column is added with correct value
            assert (
                modified_exposure.location.dataframe["BITIV"] == 0
            ).all()  # check default is applied
            assert (
                modified_exposure.ri_info.dataframe["RiskLevel"] == ""
            ).all()  # check it works for string
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                with urllib.request.urlopen(
                    base_url + "/SourceLocOEDPiWind10.csv"
                ) as response, open(
                    pathlib.Path(tmp_dir, "SourceLocOEDPiWind10.csv"), "wb"
                ) as out_file:
                    shutil.copyfileobj(response, out_file)
                with urllib.request.urlopen(
                    base_url + "/SourceAccOEDPiWind.csv"
                ) as response, open(
                    pathlib.Path(abs_dir, "SourceAccOEDPiWind.csv"), "wb"
                ) as out_file:
                    shutil.copyfileobj(response, out_file)
                ...
                original_exposure = OedExposure(
                    **{
                        ...
                        "account": str(abs_dir) + "/SourceAccOEDPiWind.csv",
                    }
                )  # absolute path
    ...
    def test_setting_schema_analysis__is_valid(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with urllib.request.urlopen(file_url) as response, open(
                pathlib.Path(tmp_dir, "analysis_settings.json"), "wb"
            ) as out_file:
                shutil.copyfileobj(response, out_file)
    ...
    def test_setting_schema_analysis__is_invalid(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            valid, errors = ods_analysis_setting.validate(
                settings_dict, raise_error=False
            )
            self.assertFalse(valid)
            self.assertEqual(
                {
                    "gul_output": ["'True' is not of type 'boolean'"],
    ...
    def test_setting_schema_model__is_valid(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with urllib.request.urlopen(file_url) as response, open(
                pathlib.Path(tmp_dir, "model_settings.json"), "wb"
            ) as out_file:
                shutil.copyfileobj(response, out_file)
            ...
            self.assertEqual(
                {
                    "additionalProperties": [
                        "Additional properties are not allowed ('model_setting', 'unknown_key' were unexpected)"
                    ],
    ...
    # A Mock OedSchema class for simulating the schema with versioning.
    class MockOedSchema:
        schema = {
            "versioning": {
                "3.0": [
    ...
    def setUp(self):
        self.data = {
            "LocNumber": [100030534294, 100030534295, 100030534296],
        ...
        self.oed_exposure = OedExposure()
        self.oed_exposure.location = MockDataFrame()
        self.oed_exposure.oed_schema = self.MockOedSchema()
    ...
    def test_oed_exposure_invalid_version(self):
        with self.assertRaises(ValueError) as context:
            self.oed_exposure.to_version("4.0")

        self.assertTrue(
            "Version 4.0 is not a known version present in the OED schema."
            in str(context.exception)
        )

    def test_oed_exposure_invalid_version_format(self):
        with self.assertRaises(ValueError) as context:
